...
export function CircularLoader({ className, size = "md" }: LoaderProps) {
...
export function ClassicLoader({ className, size = "md" }: LoaderProps) {
  ...
  const barSizes: Record<LoaderSize, { height: string; width: string }> = {
...
export function PulseDotLoader({ className, size = "md" }: LoaderProps) {
  const sizeClasses: Record<LoaderSize, string> = {
    sm: "size-1",
    md: "size-2",
    lg: "size-3",
...
export function WaveLoader({ className, size = "md" }: LoaderProps) {
  const barWidths: Record<LoaderSize, string> = {
    sm: "w-0.5",
    md: "w-0.5",
    lg: "w-1",
  };
...
export function TerminalLoader({ className, size = "md" }: LoaderProps) {
  const cursorSizes: Record<LoaderSize, string> = {
    sm: "h-3 w-1.5",
    md: "h-4 w-2",
    lg: "h-5 w-2.5",
  };

  const textSizes: Record<LoaderSize, string> = {
    sm: "text-xs",
    md: "text-sm",
    lg: "text-base",
  };
...
export function TextDotsLoader({ className, text = "Thinking", size = "md" }: LoaderProps) {
  const textSizes: Record<LoaderSize, string> = {
    sm: "text-xs",
    md: "text-sm",
    lg: "text-base",
